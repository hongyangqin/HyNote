...
上面的例子中,隐变量或者说中间变量$z$是已知的, 但$z$往往是未知的,那么我们要怎么进行参数的估计?
...
直接使用后验概率,它的表达式为:
$L(\theta|Y,Z) = \displaystyle{\prod_{i = 1}^m} P(y_i,z_i) = \displaystyle{\prod_{i = 1}^m}  \displaystyle{\sum_{z = 0}^1}
p(z|y_i,\theta^{(t)})P(y_i,z)$
取对数之后为:$LL(\theta|Y,Z) = \ln \displaystyle{\prod_{i = 1}^m} P(y_i,z_i) = \displaystyle{\sum_{i = 1}^m} \ln \displaystyle{\sum_{z = 0}^1}
p(z|y_i,\theta^{(t)})P(y_i,z)$
...
4. 重复2,3步骤,直到达到停止条件
		...
		$|\theta^{(t+1)} - \theta^{(t)}| < \epsilon_1$ 或$E_{Z|Y,\theta^{(t)}}\big( LL(\theta^{(t+1)}|Y,Z) \big) - E_{Z|Y,\theta^{(t)}}\big( LL(\theta^{(t)}|Y,Z) \big)$

至此,就可以得到参数的估计值了。

## EM算法的证明
为什么EM算法这样迭代是有效的?
$\begin{align}\ln P(Y|\theta) &= \frac{\ln P(Y,Z|\theta)}{\ln P(Z|Y,\theta)} &,贝叶斯定理\\\\
