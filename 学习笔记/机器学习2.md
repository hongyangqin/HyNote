...
$ \displaystyle{\sum_{j = 1}^m} \displaystyle{\sum_{l = 1}^k} p_{\mathcal{M}}(z_j = l|x_j,\theta^{(t)}) \ln P_{\mathcal{M}}(x_j.z_j = l|theta_l)$
...
\frac{\alpha_l}{P_{\mathcal{M}}(x_j,z_j = l|\theta_l)} \dot
\frac{\partial P(x_j|\mu_l,\Sigma_l) }{\partial \mu_i} &(1)\\
&= \displaystyle{\sum_{j = 1}^m}  p_{\mathcal{M}}(z_j = i|x_j,\theta^{(t)})
\frac{\alpha_i}{P_{\mathcal{M}}(x_j,z_j = i|\theta_i)} \dot
\frac{\partial P(x_j|\mu_i,\Sigma_i) }{\partial \mu_i} &(2)\\
&= \displaystyle{\sum_{j = 1}^m}  p_{\mathcal{M}}(z_j = i|x_j,\theta^{(t)})
\frac{\alpha_i}{P_{\mathcal{M}}(x_j,z_j = i|\theta_i)} \dot
P(x_j|\mu_i,\Sigma_i)\Sigma_{-1}(x_j-\mu_i) &(3) \\
&= \displaystyle{\sum_{j = 1}^m}  p_{\mathcal{M}}(z_j = i|x_j,\theta^{(t)}) \dot
\Sigma_{-1}(x_j-\mu_i) &(4)\\
&= 0
\end{align}$
...
&= \frac{\partial \frac{1}{t}e^p}{\parital Sigma_i} &(1)\\
&= e^p\frac{\partial \frac{1}{t}}{\parital Sigma_i} + \frac{1}{t}\frac{\partial e^p}{\parital Sigma_i} &(2)\\
\end{align}$

得:$\begin{align}
(\Sigma_i^{-1})^T\Sigma_i^2 &= \Sigma_i &(1)\\
\frac{displaystyle{\sum_{j = 1}^m}  p_{\mathcal{M}}(z_j = i|x_j,\theta^{(t)})(x_j - \mu_i)(x_j - \mu_i)^T }{displaystyle{\sum_{j = 1}^m}  p_{\mathcal{M}}(z_j = i|x_j,\theta^{(t)}) }
\end{align}$
(1):$\Sigma_i$是对称矩阵,并且$(A^{-1})^T = (A^T)^{-1}$
