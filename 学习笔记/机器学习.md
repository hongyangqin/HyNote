...
为明确显示高斯分布与相应参数的以来关系，将概率密度函数记为$p(\boldsymbol x | \boldsymbol \mu,\Sigma) = p(\boldsymbol x)$
...
对于$alpha_i$，除了最大话$LL(D)$，还有约束条件:$\alpha_i \geq 0 ,\displaystyle{\sum_{i = 1}^k} = 1$，因此考虑$LL(D)$的拉格朗日形式：$LL(D) F+ \lambda\big( \displaystyle{\sum_{i = 1}^k} -1  \big)$
...
由上述的推导即可获得搞死混合模型的EM算法：在每步迭代中，先根据当前参数来计算每一个样本属于每个高斯成分的厚颜概率$\gamma_{ji}$(E步),再根据上面推导的公式来更新模型参数 $\{(\mu_i,\Sigma_i,\alpha_i)|1 \leq i \leq k \}$(M步)
